...
initial 
begin
	state = `RESET;         // Начальное состояние FSM
	baud_flag = 0;          // Инициализация флага
end
	...
	case(state)
		`RESET: 
			begin
				bit_counter <= 0; 	
				baud_counter <= 0;
				UART_RX_Idle <= 1;  // Установка сигнала ожидания
				UART_RX_Data_Out <= 0;
				UART_RX_Ready_Out <= 0;
		...
		`WAIT_START_BIT: 
			...
			if (baud_flag) 
				if (bit_counter == 9)
					begin
						if (rx) 
							UART_RX_Ready_Out <= 1;  // Установка флага готовности данных
						state <= `RESET;
					end
                        else 
					begin
...
if (baud_counter == CLOCK_RATE / BAUD_RATE / 2)
	...
	begin
		baud_flag <= 0;      // Сброс флага	
		baud_counter <= baud_counter + 1;  // Продолжение подсчёта
	end
